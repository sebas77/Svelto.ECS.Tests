...
{
    delegate FasterDictionaryTest<uint, T> ForceUnmanagedCast<T>(uint size) where T : IEntityComponent;
    ...
    static class ConvertStructToUnmanaged<T>  where T : IEntityComponent
    {
        ...
        static class Trick
        {    
            public static FasterDictionaryTest<uint, U> ForceUnmanaged<U>(uint size) where U : unmanaged, T, IEntityComponent
            {
                return new FasterDictionaryTest<uint,U>(size, new NativeAllocationStrategy<U>());
            }
    ...
    sealed class TypeSafeDictionary<TValue> : ITypeSafeDictionary<TValue> where TValue : struct, IEntityComponent
    {
        ...
        public TypeSafeDictionary(uint size, bool IsUnmanaged)
        {
            ...
            if (IsUnmanaged)
                _implementation = new FasterDictionaryTest<uint, TValue>(size, new ManagedAllocationStrategy<TValue>());
            else
       ...
       , in PlatformProfiler profiler, EGID egid)
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public FasterDictionaryTest<uint, TValue>.FasterDictionaryKeyValueEnumerator GetEnumerator()
        {
        ...
        internal FasterDictionaryTest<uint, TValue> implementation => _implementation;
        readonly FasterDictionaryTest<uint, TValue> _implementation;
        bool _isUnmanaged;
    }
}